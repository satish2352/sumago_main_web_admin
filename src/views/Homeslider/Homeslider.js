...
const Homeslider = () => {
  ...
  useEffect(() => {
    axios.get("/homeslider/gethomeslider").then((result) => {
      setData(result.data);
    }).catch((err) => {
      console.log("err", err);
    });
  ...
  const handleDelete = (sliderId) => {
    axios.delete(`/homeslider/deletehomesliderRecord/${sliderId}`)
      .then((response) => {
  ...
  return (
    <PageContainer title="Home Slider" description="this is Sample page">
      <DashboardCard title="Home Slider" buttonName={show ? "View Sliders" : "Add Slider"} onClick={show ? onClick1 : onClick}>
        {show ? (
          ...
          <form onSubmit={SubmitForm}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField fullWidth label="Text" value={text} onChange={(e) => setText(e.target.value)} variant="outlined" />
                {errors.text && <span className="error" style={{ color: 'red' }}>{errors.text}</span>}
              </Grid>
